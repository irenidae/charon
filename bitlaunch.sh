...
wait_health() {
    ...
    for ((i=0; i<timeout; i++)); do
        id="$(${SUDO:-} docker ps -a --filter "name=^${name}$" --format '{{.ID}}' | head -n1)"
        if [[ -n "$id" ]]; then
...
#docker exec -it exit_a /bin/bash -c 'nyx --socket /run/tor/control.sock'
#docker exec -it exit_b /bin/bash -c 'nyx --socket /run/tor/control.sock'